    ...
    public void loop() {
        ...
        if (imageReady()) { // only do this if an image has been returned from the camera
            ...
            int greenValue = 0;
             bCounter =0;
             rCounter =0;
            int[][] colorArray = new int[width/ds2] [height/ds2];
            ...
            for (int x = 0; x < width / ds2; x++) {
                for (int y = 0; y < height / ds2; y++) {
                    ...
                    for (int x1 = 0; x1 <= 2; x1++) {
                        if ((oldcolors[0] + oldcolors[1] + oldcolors[2] > 370) && (oldcolors[0] + oldcolors[1] + oldcolors[2] < 724)
                                && x1 == 0) {
                            // newColors[1] = 100;
                            // newColors[2] = 120;
                            // newColors[0] = 100;
                        ...
                        if ((oldcolors[0] + oldcolors[1] + oldcolors[2] > 375) && oldcolors[x1] > 220
                        ...
                        if ((oldcolors[0] + oldcolors[1] + oldcolors[2]) > 730) {
                            newColors[1] = 0;
                            newColors[2] = 0;
                            newColors[0] = 0;
                        }
                    }
                     bCounter = bCounter + (newColors[2] / 255);
                     rCounter = rCounter + (newColors[0] / 255);
            ...
            telemetry.addData("RedX:  ",  ColorFunctions.relativeRed);
            telemetry.addData("BlueX:  ",  ColorFunctions.relativeBlue);
            ColorFunctions.FindxStart( colorArray,   width/ds2,  height/ds2);
        }
        long endTime = System.currentTimeMillis();
        telemetry.addData("Dims", Integer.toString(width / ds2) + " x " + Integer.toString(height / ds2));
        telemetry.addData("Blue", bCounter);
        telemetry.addData("Red", rCounter);
